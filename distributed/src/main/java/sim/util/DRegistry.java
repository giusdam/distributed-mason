...
public class DRegistry
{
	...
	{
		// System.out.println(obj +" $$$ " + exported_objects);
		String name = (obj instanceof DistributedIterativeRepeat)?
						 exported_objects.get(((DistributedIterativeRepeat)obj).getSteppable())
						 : exported_objects.get(obj);
		if (name != null)
...
*/
	...
	public boolean registerObject(String name, Remote obj) throws AccessException, RemoteException
	{
		if (!exported_names.containsKey(name))
		{
			try
			{
				...
				{
					System.out.println("DRegistry register " + name + " "+obj);
	...
	public boolean registerObject(String name, UnicastRemoteObject obj) throws AccessException, RemoteException
	{
		if (!exported_names.containsKey(name))
		{
			try
			{
				...
				exported_names.put(name, obj);
				exported_objects.put(obj, name);
				// System.out.println("################ Putting " + obj + name);
	...
	public boolean unRegisterObject(String name) throws AccessException, RemoteException, NotBoundException
	{
		...
		if (remote != null)
		{
			System.out.println("Unbind object "+name + " remote " + remote);
			registry.unbind(name);
			
			boolean a = UnicastRemoteObject.unexportObject(remote, false);
			
			if(!a){
				System.out.println("Object not unexported!");
				System.exit(-1);
			} 
