...
public class Partition
{
	...
	void initUniformly()
	{
		...
		// we need this because we merge the adjacent leaves so we do not have
		// leaves ID from 0 to numProcessors but we miss some ID
		// e.g. with 8 processors leaves 5 and 6, and 7 and 8 will be merged
		// so the qt nodes will be 0, 1, 2, 3, 4, 5, 7
		// this create problem with the balance partition 
		// traverse the tree and re assign the leaves ID
		renameQuadtree(numProcessorsOld);
		System.out.println("pid: " + getPID() + " qt2 --- " + qt + "........");
	...
	protected void renameQuadtree(int P){
		int newID = 0;
		for (int i = 0; i < qt.getDepth() + 1; i++) {
			final List<QuadTreeNode> nodes = qt.getAllNodes();
			for (final QuadTreeNode node : nodes) {
				if (node.getLevel() == i) {
					QuadTreeNode n = qt.getNode(node.id);
					n.setId(newID);
					newID++;
				}
	...
	private void refinePartition()
	{
		...
		// System.out.println("maxul -- " + maxul + "minbr --- " + minbr);
	...
	public void balance(final double myRuntime, final int level) throws MPIException
	{
		...
		if (gc != null)
		{
			...
			final double[] sendData = new double[3]; // dimensions (x, y) + runtime
			final double[] recvData = new double[3]; // dimensions (x, y) + runtime

			sendData[0] = myRuntime;
			sendData[1] = ctr.x * myRuntime;
			sendData[2] = ctr.y * myRuntime;
			// System.out.println("PID " + getPID() + " --- BEFORE ctr.x " + ctr.x + "--- ctr.y " + ctr.y + "--- runtime " + myRuntime);
			...
			if (isGroupMaster(gc))
			{
				...
				// System.out.println("PID " + getPID() + " --- AFTER ctr.x: " + recvData[1] + "--- ctr.y: " + recvData[2]+ "--- runtime: " + recvData[0]);
				...
				for (int i = 1; i < recvData.length; i++)
				{
					...
					// System.out.println("PID" + getPID() + "-- locVals[i - 1]: " + locVals[i - 1]);

				}
				sendCentroids = new Object[] { gc.master.getId(), new Int2D(locVals) };
				// System.out.println("Centroid sent: " + Arrays.toString(locVals));
			}
		}
		...
		// refineQuadtree(numProcessors);
		// if (isGroupMaster(gc))
		// 	System.out.println("\nAFTER REFINE 1 qt2 --- " + qt + "........");

		// refineQuadtree(numProcessors);
		// if (isGroupMaster(gc))
		// 	System.out.println("\nAFTER REFINE 2 qt2 --- " + qt + "........");


		//renameQuadtree(numProcessors);
		// if (isGroupMaster(gc))
		// 	System.out.println("\nAFTER RENAME qt2 --- " + qt + "........");
